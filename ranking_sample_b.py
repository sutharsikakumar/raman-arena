"""
============================================================
  HEXAGONAL BORON NITRIDE (h-BN) ELO RANKING SYSTEM
  Judge: Claude claude-sonnet-4-6 (CoT prompting)
  Material: Hexagonal Boron Nitride (hbn_bulk_001)
  Folder: ranking_sample_b / subfolders a–i
...
# ── Hard-coded ground truth about the h-BN sample ──────────────────────────
HBN_GROUND_TRUTH = """
...
*same* hexagonal boron nitride (h-BN) Raman spectrum.
...
Step 1 – Scientific correctness
  • Did each candidate correctly identify the material as hexagonal boron
    nitride (h-BN)?
  • Did they correctly identify the E₂g band (~1366 cm⁻¹) as the primary
    Raman-active mode?
  • Are the reported peak positions, FWHM values, and band assignments
    consistent with the ground truth? Flag any errors or confusions with
...
Step 2 – Reasoning quality (chain-of-thought)
  • Is the reasoning methodical, logically structured, and grounded in Raman
    physics specific to h-BN?
  • Does the chain-of-thought correctly distinguish h-BN from graphene
    (e.g., single dominant peak vs G+2D, different peak position)?
  • Penalise hallucinations, circular reasoning, or missing steps.

Step 3 – Completeness & clarity of classification result
  • Does the final classification result clearly state: material (h-BN),
    morphology (bulk vs monolayer), confidence, and key evidence
    (E₂g position, FWHM, absence of defect bands)?
...
def load_models(root: pathlib.Path) -> Dict[str, Dict]:
    """
    Expect: root/ranking_sample_b/{a,b,c,...}/
               chain_of_thought.json
               classification_result.json
    """
    sample_dir = root / "ranking_sample_b"
...
def run_games(
    ...
    print("\n" + "=" * 64)
    print(" HEXAGONAL BORON NITRIDE ELO RANKING SYSTEM")
...
def _build_report(
    ...
    lines = [
        ...
        "",
        f"**Sample:** hbn_bulk_001 (Bulk h-BN, orientation 001)  ",
...
def _plot(ranking: List[EloPlayer], out_dir: pathlib.Path):
    ...
    # Colour: gold → silver → bronze → teal gradient (teal for h-BN theme)
    palette = []
    for i in range(n):
        if   i == 0: palette.append("#FFD700")
        elif i == 1: palette.append("#C0C0C0")
        elif i == 2: palette.append("#CD7F32")
        else:        palette.append("#2ABEAA")  # teal to reflect h-BN's pale/white crystal
    ...
    ax.set_title(
        "h-BN Classification — ELO Rankings\n"
        f"Sample: hbn_bulk_001 · Judge: {MODEL}",
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="h-BN ELO ranking — Claude judge"
    )
    parser.add_argument(
        "--root",
        type=pathlib.Path,
        default=pathlib.Path("."),
        help="Folder that contains 'ranking_sample_b/' (default: current dir)"
    )
    parser.add_argument(
        "--output",
        type=pathlib.Path,
        default=pathlib.Path("hbn_elo_results"),
        help="Where to write results (default: ./hbn_elo_results)"
    )
